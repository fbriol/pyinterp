...
template <>
struct tag<pyinterp::geodetic::Segment> {
  using type = segment_tag;
};

template <>
struct point_type<pyinterp::geodetic::Segment> {
  using type = pyinterp::geodetic::Point;
};

/// @brief Indexed access to segment endpoints and coordinates
template <std::size_t Index, std::size_t Dim>
struct indexed_access<pyinterp::geodetic::Segment, Index, Dim> {
  static auto get(const pyinterp::geodetic::Segment& s) -> double {
    return s.template get<Index, Dim>();
  }
  static void set(pyinterp::geodetic::Segment& s, double v) {
