...
template <>
struct tag<pyinterp::geodetic::Point> {
  using type = point_tag;
};

template <>
struct coordinate_type<pyinterp::geodetic::Point> {
  using type = double;
};

template <>
struct coordinate_system<pyinterp::geodetic::Point> {
  using type = cs::geographic<degree>;
};

template <>
struct dimension<pyinterp::geodetic::Point> : boost::mpl::int_<2> {};

template <std::size_t I>
struct access<pyinterp::geodetic::Point, I> {
  static auto get(const pyinterp::geodetic::Point& p) -> double {
    return p.get<I>();
  }
  static void set(pyinterp::geodetic::Point& p, double v) { p.set<I>(v); }
};
