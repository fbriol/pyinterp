...
/// @brief Python wrapper for geodetic::Coordinates
class Coordinates : public geodetic::Coordinates {
 public:
  using geodetic::Coordinates::Coordinates;
    ...
    parallel_for(
        ...
        [&](size_t start, size_t end) {
          ...
          for (const auto &[x_i, y_i, z_i, lon_i, lat_i, alt_i] :
            ...
            auto lla = geodetic::Coordinates::ecef_to_lla<T>({x_i, y_i, z_i});
        ...
        [&](size_t start, size_t end) {
          ...
          for (const auto &[lon_i, lat_i, alt_i, x_i, y_i, z_i] :
               std::views::zip(lon_segment, lat_segment, alt_segment, x_segment,
                               y_segment, z_segment)) {
            auto ecef =
                geodetic::Coordinates::lla_to_ecef<T>({lon_i, lat_i, alt_i});
  ...
  /// system
  template <std::floating_point T>
  [[nodiscard]] auto transform(const geodetic::Coordinates &target,
        ...
        [&](size_t start, size_t end) {
          ...
          for (const auto &[lon_i, lat_i, alt_i, lon_o_i, lat_o_i, alt_o_i] :
               std::views::zip(lon_segment, lat_segment, alt_segment,
                               ...
                               alt_out_segment)) {
            auto lla = geodetic::Coordinates::transform<T>(
  ...
  static auto setstate(nanobind::tuple &state) -> Coordinates {
    ...
    return Coordinates{geodetic::Spheroid(semi_major_axis, flattening)};
  }
};
