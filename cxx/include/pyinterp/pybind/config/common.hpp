...
struct AxisConfig {
  ...
  AxisMethod method{AxisMethod::kLinear};
  ...
  constexpr explicit AxisConfig(AxisMethod method) : method(method) {}
...
class Base {
 ...
 public:
  ...
  [[nodiscard]] constexpr auto with_bounds_error(bool value) const -> Derived {
    auto copy = derived();
    copy.common = copy.common.with_bounds_error(value);
    return copy;
  }
  ...
  [[nodiscard]] constexpr auto with_num_threads(size_t value) const -> Derived {
    auto copy = derived();
    copy.common = copy.common.with_num_threads(value);
    return copy;
  }
